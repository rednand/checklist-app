    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-6 md:px-8 md:py-7 shadow-md">
        ...
        <div className="relative z-10">
          <p className="text-blue-200 text-xs md:text-sm font-medium mb-1">Bem-vindo de volta 👋</p>
          <h1 className="text-xl md:text-2xl font-bold text-white">{firstName}</h1>
          <p className="text-blue-100 text-xs md:text-sm mt-1">Pronto para organizar o dia com IA?</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
        {[
          ...
          { value: totalItems ?? 0, label: "Itens gerados", icon: <ListChecks size={16} />, border: "border-t-teal-400", iconBg: "bg-teal-50", iconColor: "text-teal-500" },
          { value: checkedItems ?? 0, label: "Concluídos", icon: <CheckCircle size={16} />, border: "border-t-green-400", iconBg: "bg-green-50", iconColor: "text-green-500" },
        ].map(({ value, label, icon, border, iconBg, iconColor }) => (
          <div key={label} className={`bg-white border border-slate-200 border-t-4 ${border} rounded-2xl p-4 md:p-5 shadow-sm flex items-start justify-between col-span-1 last:col-span-2 sm:last:col-span-1`}>
            <div>
            ...
            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-xl ${iconBg} ${iconColor} flex items-center justify-center shrink-0`}>
              {icon}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 md:p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/checklists" className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-blue-50 hover:text-blue-600 transition-colors">
              <Plus size={14} />
            </Link>
          </div>
            ...
            <div className="space-y-0.5">
              {recentChecklists.map((checklist) => (
                <Link key={checklist.id} href={`/checklists/${checklist.id}`}
                  className="flex items-center gap-3 px-2 py-2.5 rounded-lg hover:bg-slate-50 group transition-colors">
