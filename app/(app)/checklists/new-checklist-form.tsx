...
import { generateChecklist, createManualChecklist, generateFromExtraction, createFromSpreadsheet } from "../../actions/checklists"
import { Loader2, Sparkles, Plus, X, ListChecks, FileText, Upload, Table } from "lucide-react"
...
export default function NewChecklistForm() {
  ...
  return (
    <div>
      ...
      <div className="flex mb-4 border-b border-slate-100">
        {tabs.map(tab => (
          <button
            ...
            title={tab.label}
            className={`flex items-center justify-center gap-1.5 pb-2.5 px-2 sm:px-1 flex-1 sm:flex-none text-[11px] font-medium transition-all border-b-2 -mb-px ${
              mode === tab.id
      ...
      {mode === "spreadsheet" && <SpreadsheetForm isPending={isPending} startTransition={startTransition} />}
      {mode === "manual" && <ManualForm isPending={isPending} startTransition={startTransition} />}
    </div>
  )
}
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      <textarea
        ...
        autoFocus
      />
      <SubmitButton isPending={isPending} label="Gerar Checklist com IA" icon={<Sparkles size={14} />} />
    </form>
  )
}

function ExtractForm({ isPending, startTransition }: { isPending: boolean; startTransition: (fn: () => void) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      {inputMode === "file" ? (
        <label className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl p-6 cursor-pointer transition-colors ${fileName ? "border-blue-300 bg-blue-50" : "border-slate-200 hover:border-blue-300 hover:bg-blue-50/50"}`}>
          <Upload size={20} className={fileName ? "text-blue-500" : "text-slate-400"} />
      ...
      <button type="submit" disabled={busy}
        ...
        {extracting ? <><Loader2 size={14} className="animate-spin" /> Lendo PDF...</>
          : isPending ? <><Loader2 size={14} className="animate-spin" /> Gerando checklist...</>
...
async function parseSpreadsheet(file: File | null, rawText: string): Promise<{ headers: string[]; rows: ParsedRow[] }> {
  if (file) {
...
function SpreadsheetForm({ isPending, startTransition }: { isPending: boolean; startTransition: (fn: () => void) => void }) {
  ...
  const handleParse = async (file: File | null) => {
    ...
    try {
      ...
      if (result.headers.length > 0) {
        const topicCol = result.headers.find(h => /tópico|topic|item|tarefa|assunto|descrição/i.test(h)) ?? result.headers[result.headers.length - 1]
        const catCol = result.headers.find(h => /categor|subcategor|área|area|disciplina|matéria/i.test(h)) ?? ""
        setItemCol(topicCol)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      {inputMode === "file" ? (
        <label className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl p-5 cursor-pointer transition-colors ${fileName ? "border-blue-300 bg-blue-50" : "border-slate-200 hover:border-blue-300 hover:bg-blue-50/50"}`}>
          <Table size={20} className={fileName ? "text-blue-500" : "text-slate-400"} />
          <span className="text-sm text-slate-500">{fileName ?? "Clique para selecionar CSV ou Excel"}</span>
          ...
          <input type="file" accept=".csv,.xlsx,.xls" className="hidden" disabled={busy}
            onChange={async e => {
              ...
              if (f) { setFileName(f.name); await handleParse(f) }
            }} />
        </label>
      ) : (
        <div className="space-y-2">
          <textarea ref={textRef} placeholder={"Coluna1;Coluna2;Coluna3\nValor1;Valor2;Valor3"} rows={5} disabled={busy}
          ...
          <button type="button" onClick={() => handleParse(null)} disabled={busy}
      ...
      {headers.length > 0 && (
        <>
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-500">
            <span className="font-medium text-slate-700">{rows.length}</span> linhas detectadas · <span className="font-medium text-slate-700">{headers.length}</span> colunas
          </div>

          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs font-medium text-slate-600 block mb-1">Coluna do item *</label>
              <select value={itemCol} onChange={e => setItemCol(e.target.value)} required
            ...
            <div>
              <label className="text-xs font-medium text-slate-600 block mb-1">Coluna da categoria</label>
          ...
          <input value={title} onChange={e => setTitle(e.target.value)} type="text" placeholder="Título do checklist..." required
          ...
          <SubmitButton isPending={isPending} label={`Criar Checklist (${rows.length} itens)`} icon={<Table size={14} />} />
        </>
      )}
    </form>
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="space-y-1 max-h-40 overflow-y-auto">
          {items.map((item, i) => (
            <div key={i} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
              ...
              <span className="text-sm text-slate-700 flex-1 truncate">{item}</span>
      ...
      <div className="flex gap-2">
        ...
        />
        <button type="button" onClick={addItem} disabled={!inputValue.trim()}
...
function SubmitButton({ isPending, label, icon, disabled }: { isPending: boolean; label: string; icon: React.ReactNode; disabled?: boolean }) {
  return (
    <button
      type="submit"
      ...
      className="flex items-center justify-center gap-2 w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold text-sm py-3 rounded-xl transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {isPending ? <><Loader2 size={14} className="animate-spin" /> Processando...</> : <>{icon} {label}</>}
    </button>
  )
}
